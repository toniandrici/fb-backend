...
export class ReactionCache extends BaseCache {
  ...
  public async savePostReactionToCache(
    ...
    try {
      ...
      if (type) {
        ...
        await this.client.HSET(`reactions:${key}`, 'reactions', JSON.stringify(postReactions));
      }
    } catch (error) {
      log.error(error);
  ...
  public async removePostReactionFromCache(
    ...
    try {
      ...
      const response: string[] = await this.client.LRANGE(`reactions:${key}`, 0, -1); // lrange to get data from a list in redis , (if you want to get from a set you put ZRANGE)
      ...
      multi.LREM(`reactions:${key}`, 1, JSON.stringify(userPreviousReaction)); //REMOVE PREVIOUS REACTION
      await multi.exec();
      await this.client.HSET(`reactions:${key}`, 'reactions', JSON.stringify(postReactions));
    } catch (error) {
      log.error(error);
  ...
  public async getReactionsFromCache(postId: string): Promise<[IReactionDocument[], number]> {
    try {
      ...
      const response: string[] = await this.client.LRANGE(`reactions:${postId}`, 0, -1); // lrange to get data from a list in redis , (if you want to get from a set you put ZRANGE)
